...
SELECT
	...
	LEAD(amount, 1) OVER (PARTITION BY customer_id ORDER BY order_date)
FROM orders
...
WITH cust_comp AS (
	SELECT
		...
		LAG(order_date, 1) OVER (PARTITION BY customer_id ORDER BY order_date) AS 이전주문일,
		LAG(amount, 1) OVER (PARTITION BY customer_id ORDER BY order_date) AS 이전구매금액,
		LEAD(order_date, 1) OVER (PARTITION BY customer_id ORDER BY order_date) AS 다음주문일
	FROM orders
	ORDER BY customer_id, order_date
)
SELECT
	...
	다음주문일 - 주문일 AS 다음구매간격,
	구매금액 - 이전구매금액 AS 금액변화,
	ROUND(((구매금액 - 이전구매금액)*100/이전구매금액),2)||'%' AS 금액변화율
FROM cust_comp
WHERE 고객ID != 'CUST-000001'
LIMIT 10;
