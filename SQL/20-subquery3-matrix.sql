...
FROM sales s
GROUP BY s.category
HAVING avg(total_amount)>=5*power(10, 5);

-- 인라인 뷰(View) -> 내가 만든 임의의 가상 테이블
SELECT *
FROM (
  SELECT 
    category,
...
SELECT
 s.category AS 카테고리명,
 count(s.id) AS 주문건수,
 sum(s.total_amount) AS 총매출,
 avg(s.total_amount) AS 평균매출,
 CASE
  WHEN avg(s.total_amount)>=8*power(10, 5) THEN '고단가'
  WHEN avg(s.total_amount)>=4*power(10, 5) THEN '중단가'
  ELSE '저단가'
END AS 단가구분
FROM sales s
GROUP BY s.category;

-- avg(s.total_amount) 계속 반복되는 데 계속 함수를 써야해? 다른데 저장해 두었다가 쓸 수는 없나?
