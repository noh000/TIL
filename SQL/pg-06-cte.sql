...
WITH customers_s AS(
	SELECT
		c.customer_id AS 고객,
		COALESCE(SUM(o.amount), 0) AS 고객별총구매액,
		COALESCE(COUNT(o.order_id), 0) AS 고객별총주문수,
		(SELECT SUM(amount) / COUNT(DISTINCT customer_id) FROM orders) AS 평균고객별총구매액
	FROM customers c
	LEFT JOIN orders o ON c.customer_id=o.customer_id
...
customers_g AS (
	SELECT
		...
		고객별총주문수,
		CASE
			WHEN 고객별총구매액 >= 평균고객별총구매액 THEN '일반'
			ELSE '신규'
		END AS 등급
	FROM customers_s cs
)
