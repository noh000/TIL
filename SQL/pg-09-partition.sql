...
WITH prod_sales AS (
	SELECT
		...
		p.price AS 단가,
		COUNT(o.order_id) AS 상품별주문건수,
		SUM(o.quantity) AS 상품별판매개수,
		SUM(o.amount) AS 상품별총매출
	FROM products p
...
ranking AS(
	SELECT
		카테고리, 상품명, 단가, 상품별주문건수, 상품별판매개수, 상품별총매출,
		RANK() OVER(PARTITION BY 카테고리 ORDER BY 상품별총매출 DESC) AS 매출순위,
		RANK() OVER(PARTITION BY 카테고리 ORDER BY 상품별판매개수 DESC) AS 판매량순위
	FROM prod_sales
)
SELECT
	카테고리, 상품명, 단가, 상품별주문건수, 상품별판매개수, 상품별총매출, 매출순위
FROM ranking
WHERE 매출순위 <= 5;